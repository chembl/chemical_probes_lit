...
def main(input_path, probes_path):
  #Loading ner data with triplets
  ner_data = pd.read_csv(input_path, sep='\t')

  #Loading main HQ probes dataset and extracting list of genes
  hqprobes = pd.read_csv(probes_path, sep="\t")

  # mapping genes to ENSEMBL ids
  ...
  # Writing output file
  subset.to_csv('data/ner_probes_triplets_ptpairs.csv', sep='\t', index=False)
